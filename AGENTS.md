...
- 本リポジトリは、アンケート作成/回答/集計 UI を対象にした実運用想定の画面設計を目指す。
...
- 画面構成は「回答者向け画面」と「管理者向け画面」を明確に分離する。
- 画面文言、README、HTMLタイトル等では `プロトタイプ` / `preview` といった試作表現を避け、実運用を想定した表現を用いる。
- 回答確認や完了などの細分化よりも、主要導線（回答入力・送信・集計確認）を優先して設計する。
- 新しい要求が継続的に示された場合は、再発防止のためにこのAGENTS.mdへ反映する。
